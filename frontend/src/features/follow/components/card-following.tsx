...
export function CardFollow({ data }: { data: FollowerEntity }) {
  ...
  return (
    ...
    >
      <Box display="flex" alignItems="center" gap="10px">
        <Avatar size="md" src={data?.followed?.photoProfile} />
        <Box>
          <Text fontSize="12px" fontWeight="bold">
            {data?.followed?.fullName}
          </Text>
          <Text fontSize="12px">@{data?.followed?.username}</Text>
          <Text fontSize="12px">{data?.followed?.bio}</Text>
        </Box>
      </Box>
      <Box>
        <Button
